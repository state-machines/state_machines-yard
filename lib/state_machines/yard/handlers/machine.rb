    ...
    module Handlers
      ...
      class Machine < Base
        ...
        def process
          ...
          # Create new machine
          klass = if inherited_machine
                    Class.new(inherited_machine.owner_class)
                  else
                    Class.new do
          ...
          if machine.renderer.to_s == 'STDIORenderer'
            begin
              require 'stringio'
              io = StringIO.new
              namespace['state_machines'][name][:image] = io.string if machine.draw(io: io)
            ensure
              io.close
            end
          end
        ...
        def name
          @name ||= begin
            ...
            if ast && %i[symbol_literal string_literal].include?(ast.type)
        ...
        def options
          @options ||= begin
            ...
            if !inherited_machine && ast && !%i[symbol_literal string_literal].include?(ast.type)
              ast.children.each do |assoc|
                # Only extract important options
                key = extract_node_name(assoc[0])
                next unless %i[initial attribute namespace action].include?(key)
        ...
        def state_type
          @state_type ||= if machine.states.any?
                            machine.states.map(&:name).compact.first.class.to_s
                          else
                            'Symbol'
                          end
        end
        ...
        def define_macro_methods
          ...
          register(m = YARD::CodeObjects::MethodObject.new(namespace, "human_#{machine.attribute(:event_name)}",
                                                           :class))
          m.docstring = [
        ...
        def define_event_methods
          machine.events.each do |event|
            ...
            m.docstring = [
              ...
              '@option args [Array] Optional arguments to include in transition callbacks',
